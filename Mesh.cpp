	...
	name(name)
{
	...
	numVertices = (unsigned int)finalVertices.size();
	numIndices = (unsigned int)finalIndices.size();

	CalculateTangents(&finalVertices[0], numVertices, &finalIndices[0], numIndices);

	// Close the file and create the actual buffers
	obj.close();
	CreateBuffers(&finalVertices[0], numVertices, &finalIndices[0], numIndices);
}
...
void Mesh::CreateBuffers(Vertex* vertArray, size_t numVerts, unsigned int* indexArray, size_t numIndices)
{
	...
	// Create vertex and index buffer resources on the GPU
	vb = Graphics::CreateStaticBuffer(sizeof(Vertex), numVerts, vertArray);
	ib = Graphics::CreateStaticBuffer(sizeof(unsigned int), numIndices, indexArray);
